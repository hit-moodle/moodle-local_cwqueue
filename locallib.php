 ...
 * 是否在午休之前
 */
function cwq_before_break($time = -1) {
    if ($time == -1) {
        $time = time();
    }
 ...
 * 是否在午休之后
 */
function cwq_after_break($time = -1) {
    if ($time == -1) {
        $time = time();
    }
 ...
 * 返回除去午休时间的真实时间长度
 */
function cwq_real_time_interval($start, $end) {
    //暂时只当一天处理
    $offset = 0;
    if (cwq_is_break($end)) {
        $end = cwq_break_begin_time($end);
    }
    if (cwq_is_break($start)) {
        $start = cwq_break_end_time($start);
    }
    if (cwq_before_break($start) and cwq_after_break($end)) {
        $offset = 2*3600;
    }
    $interval = $end - $start - $offset;
 ...
 * 返回$time那天的午休开始时间
 */
function cwq_break_begin_time($time) {
    $t= new DateTime();
    $t->setTimestamp($time);
    $t->setTime(11, 30);
 ...
 * 返回$time那天的午休结束时间
 */
function cwq_break_end_time($time) {
    $t= new DateTime();
    $t->setTimestamp($time);
    $t->setTime(13, 30);
    return $t->getTimestamp();
